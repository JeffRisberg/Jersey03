...
public class DonorEndpointTest {
  ...
  public void testFetchList() {
    Invocation.Builder invocationBuilder = target.path("donors").request()
        .accept(MediaType.APPLICATION_JSON);

    try {
      String responseMsg = invocationBuilder.get(String.class);

      List<DonorEntity> donors = mapper
          .readValue(responseMsg, new TypeReference<List<DonorEntity>>() {
          });
  ...
  public void testFetchOne() {
    ...
    try {
      ...
      for (DonorEntity donor : donors) {
        Long id = donor.getId();
        Invocation.Builder invocationBuilder2 = target.path("donors/" + id).request()
            .accept(MediaType.APPLICATION_JSON);
  ...
  public void testFetchMissing() {
    Invocation.Builder invocationBuilder = target.path("donors/999").request()
        .accept(MediaType.APPLICATION_JSON);
  ...
  public void testDeleteValid() {
    Invocation.Builder invocationBuilder = target.path("donors/1").request()
        .accept(MediaType.APPLICATION_JSON);
